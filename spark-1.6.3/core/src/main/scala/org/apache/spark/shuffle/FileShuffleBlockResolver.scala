...
private[spark] class FileShuffleBlockResolver(conf: SparkConf)
   ...
   * when the writers are closed successfully
    *
    * 给每个map task 获取一个shuffleMap Group
   */
  def forMapTask(shuffleId: Int, mapId: Int, numReducers: Int, serializer: Serializer,
    ...
    new ShuffleWriterGroup {
      ...
      /**
        * 这里就很关键了
        * shuffle有两种模式，一种是普通的，一种是优化过的
        * 这里会判断，如果开启了consolication机制的话，也就是consolicationShuffleFiles为true的话
        * 那么实际上，不会给每个bucket都获取一个独立的文件
        * 而是为这个bucket,获取一个shuffleGroup的writer
        */
      val writers: Array[DiskBlockObjectWriter] = {
        Array.tabulate[DiskBlockObjectWriter](numReducers) { bucketId =>
          //首先，用shuffleId, mapId, bucketId来生成一个唯一的shuffleBlockId
          //然后用blockId，来调用
