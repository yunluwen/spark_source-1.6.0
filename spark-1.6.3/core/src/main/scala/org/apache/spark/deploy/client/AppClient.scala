...
/**
  * spark运行时消息通信第一步：
  * 执行应用程序需要启动sparkContext，在sparkContext启动过程中会先实例化SchedulerBackEnd对象，在独立运行（standalone）
  * 模式中实力创建的是sparkDeploySchedulerBackEnd对象,在该对象的启动中会继承父类DriverEndPoint和创建AppClient的ClientEndPoint
  * 的两个🀄️端点。
  *
  * 负责为application和spark集群进行通信
  * 会接收一个spark master的url,以及一个ApplicationDescription,和一个集群事件的监听器，以及各种事件发生监听器。
  * @param rpcEnv
  * @param masterUrls
...
private[spark] class AppClient(
  ...
  private class ClientEndpoint(override val rpcEnv: RpcEnv) extends ThreadSafeRpcEndpoint
    ...
    private def tryRegisterAllMasters(): Array[JFuture[_]] = {
      //由于HA等环境中由多个master,需要遍历所有master发送消息。
      for (masterAddress <- masterRpcAddresses) yield {
        //向线程池中启动注册线程，当该线程读到应用注册成功标示registered=true时退出注册线程
        registerMasterThreadPool.submit(new Runnable {
          override def run(): Unit = try {
            ...
            //获取master终端点的引用，发送RegisterApplication注册应用的消息。
            val masterRef =
    ...
    override def receive: PartialFunction[Any, Unit] = {
      /**
        * spark程序运行过程中的通信: 第一步 1.3
        * AppClient的ClientEndPoint接收到master发送RegisteredApplication（Application注册成功）消息，
        * 需要把注册标示registered设置为true,
