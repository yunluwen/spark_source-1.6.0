...
/**
  * 主要功能，下一个Resulttask/shuffleMapTask获取上一个shuffleMapTask的
  * 输出数据的位置信息
  */
...
private[spark] abstract class MapOutputTracker(conf: SparkConf) extends Logging {
   ...
   *
   * 向(MapOutputTrackerMasterEndPoint) trackerEndpoint发送消息GetMapOutputStatuses(shuffleId)
   */
  ...
  def getMapSizesByExecutorId(shuffleId: Int, startPartition: Int, endPartition: Int)
    ...
    // 得到元数据(上一个shuffleMapTask执行完成后的数据的位置存放信息MapStatus)
   ...
   * 获取上一个shuffleMapTask 的 MapStatus 信息
   */
