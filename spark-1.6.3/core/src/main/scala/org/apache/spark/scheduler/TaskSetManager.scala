...
/**
  * 在TaskSchedulerImpl中，对一个单独的taskSet进行调度，这个类负责追踪每一个task,
  * 如果task失败的话，会负责重试，直到超过重试次数，并且会通过延迟调度，为这个taskSet调整本地化调度机制，
  * 它的主要接口是resourceOffer,在这个接口中，taskSet会希望在一个节点上运行一个任务，并且接收任务的状态改变消息，
  * 来直到它负责的task状态改变了
  * @param sched
  * @param taskSet
...
private[spark] class TaskSetManager(
   ...
   *
   * 这个方法的大致意思就是说：会去判断这个executor在这个本地化级别，之前的等待时间是多少
   * 如果说，本地化级别的等待时间在一定范围内，那么就认为task使用本地化级别在executor上启动。
   * 这一部分是可以优化的，注意，task的分配是跟本地化级别有关的。
   */
