  ...
  extends ExecutorAllocationClient with SchedulerBackend with Logging
{
  ...
  class DriverEndpoint(override val rpcEnv: RpcEnv, sparkProperties: Seq[(String, String)])
    ...
    override def receive: PartialFunction[Any, Unit] = {
      /**
        * spark程序运行过程中的通信   第六步   6.1
        * 在TaskRunner执行任务完成时，会通过Executor的容器
        * 向DriverEndPoint终端点发送状态变更StatusUpdate消息，
        * 当DriverEndPoint终端点接收到该消息时，调用TaskSchedulerImpl的statusUpdate方法，根据
        * 任务执行不同的结果进行处理，处理完毕后再给该executor分配执行任务。
        */
      case StatusUpdate(executorId, taskId, state, data) =>
        //调用TaskSchedulerImpl的statusUpdate()方法，根据任务执行不同结果进行处理。
    ...
    override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
      /**
        * spark程序运行过程中的通信  第三步  3.2
        * DriverEndPoint接收到
        * execuotr执行容器CoarseGrainedExecutorBackend向DriverEndPoint终端点发送注册Executor的消息，
        * 作出响应
        */
      case RegisterExecutor(executorId, executorRef, hostPort, cores, logUrls) =>
        ...
        if (executorDataMap.contains(executorId)) {
          ...
          /**
            * spark任务运行过程中的通信  第五步  5.1
            * 分配运行任务资源并发送launchTask消息执行任务给Executor的容器
            *
            * 注意这里，重点来了，在这个方法中调用TaskSchedulerImpl的resourceOffers()方法，
    ...
    private def makeOffers() {
      ...
      /**
        * 传入的是Application所有可用的executor,并且将其封装成了workerOffer,每个WorkerOffer代表了
        * 每个executor可用的资源数量。
        */
      ...
      /**
        * 第一步：调用TaskSchedulerImpl的resourceOffers()方法，执行任务分配算法，将各个task分配到
        * executor上去。
        * 第二步：分配好task到executor之后，执行自己的launchTaks()方法，将分配的task发送launchTask
        * 消息到对应的executor上去，由executor启动并执行task。
        */
    ...
    /**
      ...
      * 将分配好的task发送launchTask消息到对应的executor上去，由executor启动并执行task。
      */
    private def launchTasks(tasks: Seq[Seq[TaskDescription]]) {
      for (task <- tasks.flatten) {
        ...
        if (serializedTask.limit >= akkaFrameSize - AkkaUtils.reservedSizeBytes) {
          ...
          //向executor容器CoarseGrainedExecutorBackend发送launchTask消息，来在executor上启动task
