...
/**
  * sparkContext是所有的spark应用程序的发动机引擎，就是说你想要运行spark程序就必须创建一个，
  * 不然就没的玩了。sparkContext负责初始化很多东西，当其初始化完毕以后，才能像spark集群提交任务，
  * 这个地方还有另一个管理配置的类sparkConf，它主要负责配置，检查，修改等工作，这会在后期源码阅读
  * 的时候你会经常看到的一个参数conf，说的就是它。
  *
  * spark应用程序进行各种转换操作，通过行动(action)操作触发作业运行。提交之后根据RDD之间的依赖构建DAG图,
  * DAG图提交给DAGScheduler进行解析。
  *
  * sparkContext构建的顶级三大核心：DAGScheduler，TaskScheduler，SchedulerBackend.
  *
  * DAGScheduler是面向Job的Stage的高层调度器。
  * TaskScheduler是一个接口，是低层调度器，根据具体的ClusterManager的不同会有不同的实现。
  * Standalone模式下具体实现的是TaskSchedulerlmpl。
  * SchedulerBackend是一个接口，根据具体的ClusterManger的不同会有不同的实现，
  * Standalone模式下具体的实现是SparkDeloySchedulerBackend。
  *
  * 从整个程序运行的角度来讲，sparkContext包含四大核心对象：
...
class SparkContext(config: SparkConf) extends Logging with ExecutorAllocationClient {
  ...
  try {
    ...
    /**
      * 初始化sparkContext的时候就会执行这里
      * SparkContext调用它的任何一个辅助构造函数，辅助构造函数都会去调用它的主构造函数
      * (这里的scala语法需要学习)
      */
    ...
    //消耗(最优、最小）调度机制来运行job，它会将stage作为tasksets提交到底层的TaskSchedulerImpl上，
    //来在集群上运行它们（task）
    ...
    //被重新提交，一个stage内部的失败，如果不是由于shuffle文件丢失所导致的，会被TaskScheduler处理，它会多次重试
  ...
  def runJob[T, U: ClassTag](
    ...
    //对于RDD来说，他们会根据彼此之间的依赖关系形成一个有向无环图(DAG),然后把这个DAG交给DAGScheduler来处理。
    //调用DAGScheduler来进行处理。
  ...
  private def createTaskScheduler(
    ...
    master match {
      ...
      case SPARK_REGEX(sparkUrl) =>
        //创建TaskScheduler
        //TaskSchedulerImpl()底层通过操作一个SchedulerBackend,针对不同的种类的cluster
        // （standalone、yarn和mesos），调度task。
        //他也可以通过使用一个LocalBackend，并且将isLocal参数设置为true，来在本地模式下工作。
        //它负责处理一些共同的资源，比如决定多个job的调度顺序，启动检查任务执行
