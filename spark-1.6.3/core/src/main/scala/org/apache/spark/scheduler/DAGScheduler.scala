...
/**
  *
  * DAGScheduler是面向调度阶段的高层次的调度器，DAGSchedule把DAG拆分成相互依赖的调度阶段，拆分调度阶段是以RDD的依赖
  * 是否为宽依赖，当遇到宽依赖就划分为新的调度阶段。每个调度阶段包含一个或多个任务，这些任务形成任务集，提交给底层调度器
  * TaskScheduler进行调度运行。另外，DAGScheduler记录哪些RDD被存入磁盘等物化动作，同时要寻求任务的最优化调度，例如数据
  * 本地化等；DAGScheduler监控运行调度阶段过程，如果某个调度阶段运行失败，则需要重新提交该调度阶段。
  *
  * stage划分算法：很重要！！！
  * @param sc
  * @param taskScheduler
...
class DAGScheduler(
  ...
  /**
    * 获取当前stage的父stage
    * 这个方法：对一个stage,如果它的最后一个RDD的所有依赖，都是宅依赖，那么就不会创建新的stage,
  ...
  private def getMissingParentStages(stage: Stage): List[Stage] = {
    ...
    //然后进行while循环
    while (waitingForVisit.nonEmpty) {
      //对stage的最后一个RDD,调用自己方法内部定义的visit()方法
      visit(waitingForVisit.pop())
    }
    missing.toList
  }
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    try {
      ...
      //第一步：使用创建job的最后一个RDD,创建finalStage
      //并且将stage加入到DAGSCheduler的内存缓存中
    ...
    //并且导致其他所有的stage,都给放入WaitingStages等待队列中去
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        if (missing.isEmpty) {
          ...
          //将当前stage,放入waitingStages等待执行的stage队列中
          waitingStages += stage
        }
      }
    } else {
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    /**
      ...
      * 说白了，就是从stage的最后一个rdd开始，去找，哪个rdd的partition,是被cache了，或者checkpoint了
      * 那么，task的最佳位置，就是partition缓存的/checkpoint的位置
      * 因为这样的话，task就在那个节点上运行，不需要计算之前的rdd了
      */
    val taskIdToLocations: Map[Int, Seq[TaskLocation]] = try {
      stage match {
        case s: ShuffleMapStage =>
          //getPreferredLocs 调用该方法判断stage上的所有rdd是否被缓存或者checkpoint了
      ...
      stage match {
        case stage: ShuffleMapStage =>
          partitionsToCompute.map { id =>
            ...
            //给每个task计算最佳位置
            val locs = taskIdToLocations(id)   //注意这个方法，task最佳位置计算算法
    ...
    if (tasks.size > 0) {
      ...
      /**
        * 最后，针对stage的task,创建TaskSet对象，调用taskScheduler的submitTasks方法，
        * 提交TaskSet
        */
  ...
  private def getPreferredLocsInternal(
    ...
    //最后，递归调用自己，去寻找rdd的父RDd,看看对应的partition是否被缓存或者checkpoint
