...
private[deploy] class Worker(
  ...
  /**
    * 创建注册线程池 registerMasterThreadPool，把需要申请注册的请求放到该线程池中，
    * 然后通过该线程池启动线程
    * @return
    */
  ...
  private def handleRegisterResponse(msg: RegisterWorkerResponse): Unit = synchronized {
    msg match {
      case RegisteredWorker(masterRef, masterWebUiUrl) =>
        ...
        forwordMessageScheduler.scheduleAtFixedRate(new Runnable {
          override def run(): Unit = Utils.tryLogNonFatalError {
            self.send(SendHeartbeat)   //向master定时发送心跳信息
          }
        }, 0, HEARTBEAT_MILLIS, TimeUnit.MILLISECONDS)

        //如果设置清理以前应用使用的文件夹，则进行该操作
        if (CLEANUP_ENABLED) {
          logInfo(
  ...
  override def receive: PartialFunction[Any, Unit] = synchronized {
    ...
    /**
      * master 主备切换机制，接收到新的master发送过来的地址，作出响应
       */
    case MasterChanged(masterRef, masterWebUiUrl) =>
      ...
      //给新的master发送worker的信息(workerid,worker上运行的executor序列,worker上运行的driver序列)
    ...
    /**
      * spark程序运行过程中通信 第二步 2.2
      * 接收到mater发送的LaunchExecutor消息，进行处理
      */
    case LaunchExecutor(masterUrl, appId, execId, appDesc, cores_, memory_) =>
      if (masterUrl != activeMasterUrl) {
        ...
        try {
          ...
          //通过Spark_Executor_Dirs环境变量，在worker中创建Executor执行目录，当程序执行完毕后由worker自行删除。
          ...
          //在这里，创建ExecutorRunner的时候做了很多事！！！
          //在创建ExecutorRunner实例中调用了FetchAndRunExecutor方法，在该方法中command在SparkDeploySchedulerBackend
          //中定义，指定构造Executor的运行容器CoarseGrainedExecutorBackend
    ...
    //接收到master的launchDrievr的消息，作出响应，启动driver
